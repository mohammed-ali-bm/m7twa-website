...
class OrderController extends Controller
{
    ...
    public function viewQR($token)
    {
        ...
        $order = Order::where("token", $token)->with(["items" => function ($query) {
            $query->groupBy("offer_id", "order_id")->select("offer_id", "order_id",     DB::raw('COUNT(items.offer_id) as total'))->with("offer");
        }])->first();
    ...
    function store(StoreOrderRequest $request, Order $order)
    {
        ...
        if ($create == true) {
            ...
            return redirect()->route('orders.index')
                ->with('error', __($create['message'] ?? "error in order creation"));
        }
