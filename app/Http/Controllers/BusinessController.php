...
class BusinessController extends Controller
{
    ...
    public function statistics(Business $business, Request $request)
    {
        ...
        if ($from) {
            $where[] = ["redeemed_date", ">=", $from];
        }

        if ($to) {
            $where[] = ["redeemed_date", "<=", $to];
        }

        $business->due_sum =  Item::whereHas("offer", function ($q) use ($business, $where) {
            $q->where("business_id", $business->id)->where($where);
        })->where([['redeemed_date', "!=", NULL]])->sum("business_due");
        return view(
            ...
            [
                ...
                "items" =>  new Items(Item::with("offer.business")->with("order")->whereHas("offer", function ($q) use ($business) {
                    $q->where("business_id", $business->id);
                })->where([["created_at", ">=", $from], ["created_at", "<=", $to]])->orderBy("id", "DESC"), 10000000),
