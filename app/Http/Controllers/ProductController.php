...
class ProductController extends Controller
{
    ...
    public function edit(Product $product)
    {
        ...
        foreach($product->productOffer as $offer){
            $product["offer_count_".$offer->offer_id] = $offer->count;
        }
    ...
    function store(StoreProductRequest $request)
    {
        ...
        ProductOffer::insert(array_map(function ($offer_id ) use ($request) {
            return [
                "offer_id" => $offer_id,
                "count" => (int) $request->input( "offer_count_".$offer_id) > 1  ? (int) $request->input ("offer_count_". $offer_id) : 1,
