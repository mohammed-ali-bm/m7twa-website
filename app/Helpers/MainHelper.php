...
function sendSMS($mobileNumber, $messageContent)
{
    ...
    $mobileNumber = prefix_number($mobileNumber);
...
function orderQrTokenImgSrc($token)
{

    return "data:image/png;base64," . base64_encode(QrCode::size(400)->format('png')->generate(route('invitations.redeem', $token)));
}

function guestID($id)
{

    return  "2024" .  str_pad($id, 4, "0", STR_PAD_LEFT);
    ...
    return
        [
            [
                ...
                "title" => "الجلسة الأولى",
                "content" => "تحديد الاحتياج وقياس الأثر",
            ],

            [

                "title" => "الجلسة الثانية ",
                "content" => "دور الجهات المشرفة فنياً في تمكين الجمعيات الأهلية ",
            ],

            [

                "title" => "الجلسة الثالثة ",
                "content" => "دور المؤسسات المانحة في احداث الأثر ",
            ],

            [

                "title" => "الجلسة الرابعة ",
                "content" => "افاق جديدة للجمعيات الاهلية ",
            ]
...
function goalsArr()
{
    ...
    return [
        [
            "title" => "	تعزيز دور الجمعيات الأهلية في تحقيق التنمية المستدامة في المجتمع.",
            "icon" => "enhance.png",

        ],
        [
            "title" => "	مناقشة التحديات التي تواجه الجمعيات وبحث سبل حلها.",
            "icon" => "meeting.png",

        ],
        [
            "title" => "	تشجيع الابتكار في قطاع الجمعيات الأهلية.",
            "icon" => "creativity.png",

        ],
        [
            "title" => "	تعزيز التواصل بين الجمعيات الأهلية والجهات الحكومية والخاصة.",
            "icon" => "communication.png",

        ],
        [
            "title" => "	إشراك المجتمع في دعم العمل الأهلي من خلال التوعية بأهميته ودوره.",
            "icon" => "engagement.png",
...
function activeClass($val1, $val2)
{
